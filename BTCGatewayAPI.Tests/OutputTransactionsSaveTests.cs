...
namespace BTCGatewayAPI.Tests
{
    ...
    public class OutputTransactionsSaveTests
    {
        ...
        public async System.Threading.Tasks.Task Test_SaveOutputTx_Success()
        {
            ...
            var affected = await DBContext.DeleteAsync(result.Item2);

            Assert.IsTrue(affected > 0);

            wallet.Amount = 0.19552134M;

            wallet = await DBContext.UpdateAsync(wallet);

            Assert.IsTrue(wallet.Amount - 0.19552134M <= 0.0000000001M);
        }
